...
// ─── Sort ─────────────────────────────────────────────────────────────────────
type SortKey = 'ad_name' | 'adset_name' | 'impressions' | 'clicks' | 'ctr' | 'results' | 'cost_per_result' | 'cpc' | 'reach' | 'spent';
...
export default function AdsTable({ data, loading }: Props) {
  ...
  const filtered = (filter === 'all' ? [...data] : data.filter((r) => getFilterKey(r.result_type) === filter))
    .filter((r) => !searchLower || r.ad_name.toLowerCase().includes(searchLower) || r.adset_name.toLowerCase().includes(searchLower))
    .sort((a, b) => {
      const av = sortKey === 'ad_name' ? a.ad_name : sortKey === 'adset_name' ? a.adset_name : (a[sortKey] as number);
      const bv = sortKey === 'ad_name' ? b.ad_name : sortKey === 'adset_name' ? b.adset_name : (b[sortKey] as number);
    ...
    <div className="overflow-hidden">
      ...
      <div className="px-5 py-4 border-b" style={{ borderColor: 'var(--border)' }}>
        <div className="flex items-start justify-between gap-3 flex-wrap mb-3">
          <div>
            <h3 className="font-bold text-sm" style={{ color: 'var(--text)' }}>Anuncios</h3>
            <p className="text-xs mt-0.5" style={{ color: 'var(--muted)' }}>
              {filtered.length} de {data.length} anuncios · acumulado del período
              {totalSpent > 0 && (
              ...
              return (
                <tr key={`${row.id}-${i}`}>
                  ...
                  <td style={{ maxWidth: 200 }}>
                    <span className="font-semibold text-xs block truncate" style={{ color: 'var(--text)' }} title={row.ad_name}>
                      {row.ad_name}
                    </span>
                  </td>
                  ...
                  <td style={{ maxWidth: 160 }}>
                    <span className="text-xs block truncate" style={{ color: 'var(--muted)' }} title={row.adset_name}>
                      {row.adset_name}
                    </span>
                  </td>
                  ...
                  {/* Alcance */}
                  <td className="text-right font-mono text-xs" style={{ color: 'var(--muted)' }}>{fmt(row.reach)}</td>
              ...
              <tr style={{ background: 'var(--surface2)', borderTop: '2px solid var(--border)' }}>
                <td colSpan={3}>
                  <span className="font-bold text-xs" style={{ color: 'var(--text)' }}>
                    Total · {filtered.length} anuncios
                  </span>
                </td>
